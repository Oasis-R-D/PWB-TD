...
function client.tickPlayerM40(p, dt)
	...
	if InputPressed("usetool", p) and ammo > 0.5 and GetPlayerVehicle(p) == 0 and GetPlayerPickBody(p) == 0 then
			if data.coolDown < 0 then
	...
	if InputPressed("grab", p) and GetPlayerVehicle(p) == 0 and GetPlayerPickBody(p) == 0 then
		if data.altCoolDown < 0 then
