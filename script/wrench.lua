...
function client.tickPlayerWRNCH(p, dt)
	...
	--Check if firing
	if InputDown("usetool", p) and GetPlayerVehicle(p) == 0 and GetPlayerPickBody(p) == 0 and data.inAltAttack == false then
		if data.coolDown < 0 then
	...
	if InputDown("grab", p) and GetPlayerVehicle(p) == 0 and GetPlayerPickBody(p) == 0 and data.inAltAttack == false then
		if data.coolDown < 0 then
			data.inAltAttack = true
